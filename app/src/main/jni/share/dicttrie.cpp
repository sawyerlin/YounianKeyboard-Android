...
bool DictTrie::load_dict_fd(int sys_fd, long start_offset,
                            ...
                            LemmaIdType end_id) {
  __android_log_print(ANDROID_LOG_DEBUG, "LOG_TAG", "first step %s", "1");
  if (start_offset < 0 || length <= 0 || end_id <= start_id)
    return false;

  __android_log_print(ANDROID_LOG_DEBUG, "LOG_TAG", "second step %s", "1");
  FILE *fp = fdopen(sys_fd, "rb");
  if (NULL == fp)
    return false;

  __android_log_print(ANDROID_LOG_DEBUG, "LOG_TAG", "third step %s", "1");
  ...
  __android_log_print(ANDROID_LOG_DEBUG, "LOG_TAG", "forth step %s", "1");
  dict_list_ = new DictList();
  if (NULL == dict_list_) {
    fclose(fp);
    return false;
  ...
  __android_log_print(ANDROID_LOG_DEBUG, "LOG_TAG", "fifth step %s", "1");
  if (!spl_trie.load_spl_trie(fp) || !dict_list_->load_list(fp) ||
      !load_dict(fp) || !ngram.load_ngram(fp) ||
  ...
  __android_log_print(ANDROID_LOG_DEBUG, "LOG_TAG", "sixth step %s", "1");
  fclose(fp);
  return true;
}
