...
bool MatrixSearch::init_fd(int sys_fd, long start_offset, long length,
  ...
  __android_log_print(ANDROID_LOG_DEBUG, "LOG_TAG", "sys_fd is %d", sys_fd);
  __android_log_print(ANDROID_LOG_DEBUG, "LOG_TAG", "start_offset is %d", start_offset);
  __android_log_print(ANDROID_LOG_DEBUG, "LOG_TAG", "length is %d", length);
  __android_log_print(ANDROID_LOG_DEBUG, "LOG_TAG", "fn_usr_dic is %s", fn_usr_dict);
  if (NULL == fn_usr_dict)
    return false;

  __android_log_print(ANDROID_LOG_DEBUG, "LOG_TAG", "alloc_resource %s", "1");
  if (!alloc_resource())
    return false;

  __android_log_print(ANDROID_LOG_DEBUG, "LOG_TAG", "end alloc_resource %s", "1");
  __android_log_print(ANDROID_LOG_DEBUG, "LOG_TAG", "dict_trie load_dict_fd %s", "1");
  if (!dict_trie_->load_dict_fd(sys_fd, start_offset, length, 1, kSysDictIdEnd))
    return false;

  __android_log_print(ANDROID_LOG_DEBUG, "LOG_TAG", "end dict_trie load_dict_fd %s", "1");
  if (!user_dict_->load_dict(fn_usr_dict, kUserDictIdStart, kUserDictIdEnd)) {
    delete user_dict_;
    user_dict_ = NULL;
  } else {
