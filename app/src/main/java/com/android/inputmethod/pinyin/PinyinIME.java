...
import com.android.inputmethod.pinyin.source.Cycle;
import com.android.inputmethod.pinyin.source.Source;
import com.android.inputmethod.pinyin.source.SourceType;
...
public class PinyinIME extends InputMethodService {
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event) {
        if (processKey(keyCode, 0 != event.getRepeatCount())) return true;
    ...
    public boolean onKeyUp(int keyCode, KeyEvent event) {
        ...
        if (processKey(keyCode, true)) return true;
        return super.onKeyUp(keyCode, event);
    }

    private boolean processKey(int keyCode, boolean realAction) {
        if (ImeState.STATE_BYPASS == mImeState) return false;

        // SHIFT-SPACE is used to switch between Chinese and English
        // when HKB is on.
        if (KeyEvent.KEYCODE_SPACE == keyCode && false) {
        ...
        if (mInputModeSwitcher.isEnglishWithSkb()) {
            return mImEn.processKey(getCurrentInputConnection(), keyCode,
            ...
            if (mImeState == ImeState.STATE_IDLE ||
                ...
                return processStateIdle(keyChar, keyCode, realAction);
            } else if (mImeState == ImeState.STATE_INPUT) {
                return processStateInput(keyChar, keyCode, realAction);
            } else if (mImeState == ImeState.STATE_PREDICT) {
                return processStatePredict(keyChar, keyCode, realAction);
    ...
    private boolean startPinyinDecoderService() {;
        System.out.println("== test mIPinyinDecoderService");
        if (null == mDecInfo.mIPinyinDecoderService) {
            System.out.println("== mIPinyinDecoderService is not null");
            ...
            if (bindService(serviceIntent, mPinyinDecoderServiceConnection,
                ...
                System.out.println("== bind service true");
                return true;
            } else {
                System.out.println("== bind service false");
                return false;
            }
        }
        return true;
    ...
    public class PinyinDecoderServiceConnection implements ServiceConnection {
        public void onServiceConnected(ComponentName name, IBinder service) {
            mDecInfo.mIPinyinDecoderService = IPinyinDecoderService.Stub
                    .asInterface(service);
            System.out.println("== onServiceConnected then get service");
        }
